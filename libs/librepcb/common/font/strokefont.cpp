...
QVector<Path> StrokeFont::stroke(const QString& text, const Length& height,
    const Length& strokeWidth, const Ratio& lineSpacingFactor, const Alignment& align,
    Point& bottomLeft, Point& topRight) const noexcept
{
    accessor(); // block until the font is loaded. TODO: abort instead of waiting?
    QVector<Path> paths;
    Length totalWidth;
    QVector<QPair<QVector<Path>, Length>> lines = strokeLines(text, height, strokeWidth, totalWidth);
...
QVector<QPair<QVector<Path>, Length>> StrokeFont::strokeLines(const QString& text,
    const Length& height, const Length& strokeWidth, Length& width) const noexcept
{
    QVector<QPair<QVector<Path>, Length>> result;
    foreach (const QString& line, text.split('\n')) {
        QPair<QVector<Path>, Length> pair;
        pair.first = strokeLine(line, height, strokeWidth, pair.second);
        result.append(pair);
...
QVector<Path> StrokeFont::strokeLine(const QString& text, const Length& height,
                                     const Length& strokeWidth, Length& width) const noexcept
{
    QVector<Path> paths;
    Length offset = 0;
    ...
    for (int i = 0; i < text.length(); ++i) {
        ...
        if (!glyphPaths.isEmpty()) {
            ...
            offset = width + glyphSpacing + calcGlobalLetterSpacing(height, strokeWidth);
        } else if (glyphSpacing != 0) {
            // it's a whitespace-only glyph -> count additional glyph spacing as width
            width = offset + glyphSpacing;
            offset = width + calcGlobalLetterSpacing(height, strokeWidth);
        }
    }
    return paths;
}
...
Length StrokeFont::calcGlobalLetterSpacing(const Length& height, const Length& strokeWidth) const noexcept
{
    // add stroke width to letter spacing to avoid overlapped glyphs caused by thick lines
    return convertLength(height, mFont->header.letterSpacing) + strokeWidth;
}
