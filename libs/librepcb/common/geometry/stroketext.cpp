...
StrokeText::StrokeText(const Uuid& uuid, const QString& layerName, const QString& text,
        ...
        const Length& strokeWidth, const Ratio& lineSpacing, const Alignment& align,
        bool mirrored, bool autoRotate) noexcept :
    mUuid(uuid), mLayerName(layerName), mText(text), mPosition(pos), mRotation(rotation),
    mHeight(height), mStrokeWidth(strokeWidth), mLineSpacingFactor(lineSpacing),
    mAlign(align), mMirrored(mirrored), mAutoRotate(autoRotate),
    mAttributeProvider(nullptr), mFont(nullptr)
{
}
    ...
    mAttributeProvider(nullptr), mFont(nullptr)
{
    ...
    if (const SExpression* child = node.tryGetChildByPath("line_spacing_factor")) {
        ...
        // backward compatibility, remove this some time!
        mLineSpacingFactor = Ratio::percent100();
    }
...
void StrokeText::updatePaths() noexcept
{
    ...
    if (mFont) {
        ...
        paths = mFont->stroke(str, mHeight, mStrokeWidth, mLineSpacingFactor, mAlign,
                              bottomLeft, topRight);
